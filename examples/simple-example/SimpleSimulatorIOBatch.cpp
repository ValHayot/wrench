...
int main(int argc, char **argv) {
  ...
  std::cerr << "Done instantiating.." << std::endl;
   ...
   */
  /* vhs modif - first host will always be the storage host */
 

  // std::cout << "test" << std::endl;
  // std::vector<simgrid::s4u::Storage*> allStorages = simulation.gets4u()->getEngine()->get_all_storages();
  ...
  std::cout << storage->get_cname() << std::endl;
  //std::cout << props->bucket_count() << std::endl;

  std::cout << storage->extension<simgrid::s4u::FileSystemStorageExt>()->get_size() << std::endl;

  //for (auto const& s : allStorages) {
  //  std::cout << s->get_cname() << std::endl;
  //  std::cout << sg_storage_get_size(s) << std::endl;
    //std::cout << s->get_cname() << std::endl;
  //}
  std::string storage_host_name = hostname_list[0];
  simgrid::s4u::Host* storage_host = simgrid::s4u::Host::by_name(storage_host_name);
  std::string size = "1000000000000000000"; //storage_host->get_property("storage_size"); 
  std::cerr << "Instantiating a SimpleStorageService on " << storage_host_name << "..." << std::endl;
  std::cout << size << std::endl;
  wrench::StorageService *storage_service = simulation.add(
          new wrench::SimpleStorageService(storage_host_name, 100000000000000.0));

  hostname_list.erase (hostname_list.begin());
